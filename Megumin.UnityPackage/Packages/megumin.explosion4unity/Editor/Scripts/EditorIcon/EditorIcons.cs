...
/// <summary>
/// 修改自 https://github.com/nukadelic/UnityEditorIcons
/// </summary>
partial class EditorIcons
{
    public TaskCompletionSource<Texture2D> Source { get; private set; }
    ...
    /// </summary>
    /// <returns></returns>
    public static Task<Texture2D> Select()
    {
        TaskCompletionSource<Texture2D> source = new TaskCompletionSource<Texture2D>();
...
public partial class EditorIcons : EditorWindow
{
    [MenuItem("Tools/Style/Editor Icons %e", priority = -1001)]
    ...
    private void OnGUI()
    {
                ...
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    for (var i = 0; i < gridW; ++i)
                    {
                            ...
                            GUILayout.Height(buttonSize)))
                        {
                            ...
                            if (SelectMode)
                            {
                                var delta = EditorApplication.timeSinceStartup - lastClickTime;
                                lastClickTime = EditorApplication.timeSinceStartup;
                                if (delta < 0.5f)
                                {
                                    var t2d = icon.image as Texture2D;
                                    Source.TrySetResult(t2d);
                                    Source = null;
                                    Close();
                                }
                            }
        ...
        using (new GUILayout.HorizontalScope(EditorStyles.helpBox, GUILayout.MaxHeight(viewBigIcons ? 140 : 120)))
        {
            ...
            using (new GUILayout.VerticalScope())
            {
                ...
                s += $"\nTotal {iconContentListAll.Count} icons    BigIcon {iconContentListBig.Count}  SmallIcon {iconContentListSmall.Count}";
                GUILayout.Space(5);
    ...
    private static readonly string[] OriList = {
        "_Help",
        "_Popup",
    ...
    public static string[] ico_list =
        //OriList;
        EditorIconListByCodeGenerate.IconList;
